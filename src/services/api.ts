...
const asArray = (data: any) => (Array.isArray(data) ? data : data != null ? [data] : []);
...
export const authAPI = {
  ...
  getCurrentUser: async (): Promise<User> => {
    const res = await api.get('/login', { validateStatus: () => true });
...
export const workoutsAPI = {
  getAll: async (): Promise<WorkoutSession[]> => {
    ...
    return asArray(r.data).map(toWorkout);
  },
  ...
  search: async (query: string): Promise<WorkoutSession[]> => {
    ...
    const r = await api.get(
      `/WorkoutSessions?where=${encodeURIComponent(`Name like '%${safe}%'`)}`
    );
    return asArray(r.data).map(toWorkout);
  }
};
...
export const exercisesAPI = {
  getAll: async (): Promise<Exercise[]> => {
    ...
    return asArray(r.data).map(toExercise);
  },
  ...
  create: async (exercise: Omit<Exercise, 'id' | 'createdBy' | 'createdAt'>): Promise<Exercise> => {
...
export const goalsAPI = {
  getAll: async (): Promise<Goal[]> => {
    ...
    return asArray(r.data).map(toGoal);
  },
  ...
  create: async (
    goal: Omit<Goal, 'id' | 'currentValue' | 'isCompleted' | 'createdAt'>
  ): Promise<Goal> => {
  ...
  update: async (id: number, goal: Partial<Goal>): Promise<Goal> => {
    const r = await api.put(`/Goals/${id}`, {
      ...(goal.userId       !== undefined ? { UserId: goal.userId } : {}),
...
export const adminAPI = {
  getAllUsers: async (): Promise<User[]> => {
    ...
    return asArray(r.data).map(toUser);
  },
